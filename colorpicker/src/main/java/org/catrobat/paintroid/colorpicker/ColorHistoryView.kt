...
class ColorHistoryView : LinearLayout {
    ...
    var colorHistory: Queue<Int> = LinkedList()
    ...
    fun addToHistory(color: Int) {
        for (colorInHistory in colorHistory) {
            if (colorInHistory == color) {
                colorHistory.remove(colorInHistory)
                break
            }
        }
        if (colorHistory.size == MAXIMUM_COLORS_IN_HISTORY) {
            colorHistory.remove()
        }
    ...
    fun updateColorHistory(updatedHistory: LinkedList<Int>) {
