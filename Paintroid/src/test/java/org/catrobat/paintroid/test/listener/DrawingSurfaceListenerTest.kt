...
class DrawingSurfaceListenerTest {
    ...
    fun testOnTouchDown() {
        ...
        Mockito.verify(callback, Mockito.never())?.translatePerspective(ArgumentMatchers.anyFloat(), ArgumentMatchers.anyFloat())
    }
    ...
    fun testOnTouchMoveScaleInPinchMode() {
        ...
        drawingSurfaceListener?.onTouch(drawingSurface, motionEvent)
        Mockito.verify(currentTool, Mockito.never())?.handleMove(ArgumentMatchers.any(PointF::class.java))
        Mockito.verify(autoScrollTask, Mockito.never())?.setEventPoint(ArgumentMatchers.anyFloat(), ArgumentMatchers.anyFloat())
        Mockito.verify(autoScrollTask, Mockito.never())?.setViewDimensions(ArgumentMatchers.anyInt(), ArgumentMatchers.anyInt())
        Mockito.verify(callback, Mockito.never())?.translatePerspective(ArgumentMatchers.anyFloat(), ArgumentMatchers.anyFloat())
    ...
    fun testTouchCorrection() {
        ...
        Mockito.verify(currentTool)
            ?.handleUp(PointFMatcher.pointFEquals(firstMovementPositionX, firstMovementPositionY)
            )
    }
