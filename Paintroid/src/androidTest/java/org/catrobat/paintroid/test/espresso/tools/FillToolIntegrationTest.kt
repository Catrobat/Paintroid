...
class FillToolIntegrationTest {
    ...
    fun testNothingHappensWhenClickedOutsideDrawingArea() {
        ...
        onToolProperties().checkMatchesColor(Color.BLACK)
        onDrawingSurfaceView().perform(UiInteractions.touchAt(DrawingSurfaceLocationProvider.OUTSIDE_MIDDLE_RIGHT))
    ...
    fun testOnlyFillInnerArea() {
        ...
        onToolBarView().performSelectTool(ToolType.FILL)
        onToolProperties().setColorResource(R.color.pocketpaint_color_picker_green1)
        onDrawingSurfaceView()
    ...
    fun testFillToolOptionsDialog() {
        ...
        fillTool?.getToleranceAbsoluteValue(DEFAULT_TOLERANCE_IN_PERCENT)?.toDouble()?.let {
            Assert.assertEquals(
        ...
        fillTool?.colorTolerance?.toDouble()?.let {
    ...
    fun testFillToolDialogAfterToolSwitch() {
        ...
        expectedAbsoluteTolerance?.toDouble()?.let {
            Assert.assertEquals(
                "Wrong fill tool member value for color tolerance",
                it,
                fillTool.colorTolerance.toDouble(),
    ...
    companion object { private const val TOLERANCE_DELTA = 0.05 }
}
