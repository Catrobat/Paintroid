...
object PaintroidAsserts {
	...
	fun assertPaintEquals(message: String?, expectedPaint: Paint, actualPaint: Paint) {
        ...
        Assert.assertEquals(
            message,
            expectedPaint.strokeWidth,
            actualPaint.strokeWidth,
            Float.MIN_VALUE
        )
    }
	...
	fun assertPathEquals(message: String?, expectedPath: Path, actualPath: Path) {
        ...
        Assert.assertEquals(
            message,
            expectedPathBounds.bottom,
            actualPathBounds.bottom,
            Float.MIN_VALUE
        )
        Assert.assertEquals(message, expectedPathBounds.top, actualPathBounds.top, Float.MIN_VALUE)
        Assert.assertEquals(
            message,
            expectedPathBounds.left,
            actualPathBounds.left,
            Float.MIN_VALUE
        )
        Assert.assertEquals(
            message,
            expectedPathBounds.right,
            actualPathBounds.right,
            Float.MIN_VALUE
        )
    }
