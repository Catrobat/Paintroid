...
class ShapeToolOrientationIntegrationTest {
    ...
    fun testRememberShapeAfterOrientationChange() {
        ...
        onView(ViewMatchers.withId(shapeId)).check(ViewAssertions.matches(ViewMatchers.isSelected()))
        onToolBarView().performCloseToolOptionsView()
        onDrawingSurfaceView().perform(UiInteractions.touchAt(DrawingSurfaceLocationProvider.TOOL_POSITION))
    ...
    fun testRememberOutlineShapeAfterOrientationChange() {
        ...
        onView(ViewMatchers.withId(shapeId)).check(ViewAssertions.matches(ViewMatchers.isSelected()))
        onView(ViewMatchers.withId(R.id.pocketpaint_shape_ibtn_outline))
        ...
        activityTestRule.activity.requestedOrientation = ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE
        onView(ViewMatchers.withId(shapeId)).check(ViewAssertions.matches(ViewMatchers.isSelected()))
        onView(ViewMatchers.withId(R.id.pocketpaint_shape_ibtn_outline)).check(ViewAssertions.matches(ViewMatchers.isSelected()))
        onToolBarView().performCloseToolOptionsView()
        onDrawingSurfaceView().perform(UiInteractions.touchAt(DrawingSurfaceLocationProvider.TOOL_POSITION))
        expectedBitmap?.sameAs(workspace?.bitmapOfCurrentLayer)?.let { Assert.assertTrue(it) }
    }
