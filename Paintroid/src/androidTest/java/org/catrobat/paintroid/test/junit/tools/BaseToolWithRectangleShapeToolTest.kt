...
class BaseToolWithRectangleShapeToolTest {
    ...
    fun testRotateRectangle() {
        ...
        val topLeftRotationPoint = PointF(
            toolPosition.x - (rectWidth / 2) - (symbolDistance / 2),
            toolPosition.y - (rectHeight / 2) - (symbolDistance / 2))
        val topRightRotationPoint = PointF(toolPosition.x + toolPosition.x - topLeftRotationPoint.x, topLeftRotationPoint.y)
        val bottomRightRotationPoint = PointF(topRightRotationPoint.x,
                                              toolPosition.y + (rectHeight / 2) + (symbolDistance / 2)
        )
    ...
    private inner class BaseToolWithRectangleShapeImpl(
        ...
        override val toolType: ToolType, toolPaint: ToolPaint?,
        layerModelWrapper: Workspace?,
        idlingResource: CountingIdlingResource?,
        commandManager: CommandManager?) : BaseToolWithRectangleShape(
