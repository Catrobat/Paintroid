...
class ClipboardToolTest {
	...
	fun setUp() {
		...
		Mockito.`when`(workspace.getCanvasPointFromSurfacePoint(any<PointF>())).then { invocation -> invocation.getArgument<PointF>(0) }
		tool = ClipboardTool(clipboardToolOptionsView!!, contextCallback, toolOptionsViewController!!, toolPaint!!, workspace, idlingResource!!, commandManager!!, 0)
	}
