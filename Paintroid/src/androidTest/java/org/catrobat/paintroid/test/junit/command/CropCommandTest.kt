...
class CropCommandTest {
    ...
    fun setUp() {
        ...
        maximumBitmapResolution = (
            bitmapUnderTest.width * bitmapUnderTest.height
        ...
        Assert.assertEquals(
            ...
            (
                heightOriginal - resizeCoordinateYTop -
                    (widthOriginal - (resizeCoordinateYBottom + 1))
                ).toLong(),
            enlargedBitmap.height.toLong()
        )
    ...
    fun testIfBitmapIsNotResizedWithInvalidBounds() {
        ...
        if (originalBitmap != null) {
            Assert.assertTrue(
                "bitmap must not change if X left is larger than bitmap scope",
                originalBitmap.sameAs(
        ...
        if (originalBitmap != null) {
            Assert.assertTrue(
                "bitmap must not change if X right is smaller than bitmap scope",
                originalBitmap.sameAs(
                    layerUnderTest.bitmap
                )
            )
        ...
        if (originalBitmap != null) {
            Assert.assertTrue(
                "bitmap must not change if Y top is larger than bitmap scope",
                originalBitmap.sameAs(
                    layerUnderTest.bitmap
                )
            )
        ...
        if (originalBitmap != null) {
            Assert.assertTrue(
                "bitmap must not change with widthXRight < widthXLeft bound",
                originalBitmap.sameAs(
                    layerUnderTest.bitmap
                )
            )
        ...
        if (originalBitmap != null) {
            Assert.assertTrue(
                "bitmap must not change with widthYBottom < widthYTop bound",
                originalBitmap.sameAs(
                    layerUnderTest.bitmap
                )
            )
