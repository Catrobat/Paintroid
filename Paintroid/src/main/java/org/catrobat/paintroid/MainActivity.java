...
public class MainActivity extends NavigationDrawerMenuActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    private void initLocaleConfiguration() {
        ...
        if (languageTag.length() == 2) {
            MultilingualActivity.updateLocale(getBaseContext(), languageTag, null);
        } else if (languageTag.length() == 6) {
            String language = languageTag.substring(0, 2);
            String country = languageTag.substring(4);
            MultilingualActivity.updateLocale(getBaseContext(), language, country);
        }
    }
}
