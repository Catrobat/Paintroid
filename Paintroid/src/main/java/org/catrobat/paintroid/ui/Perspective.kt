...
open class Perspective(bitmapWidth: Int, bitmapHeight: Int) {
    @VisibleForTesting
    @JvmField
    var surfaceWidth = 0
    ...
    val scaleForCenterBitmap: Float
        get() {
            ...
            val bitmapSizeRatio = bitmapWidth / bitmapHeight
            var ratioDependentScale = if (screenSizeRatio > bitmapSizeRatio) {
                surfaceHeight / bitmapHeight
            } else {
                surfaceWidth / bitmapWidth
            }
    ...
    private var bitmapWidth: Float
    private var bitmapHeight: Float
    private var initialTranslationX = 0f

    init {
        this.bitmapWidth = bitmapWidth.toFloat()
        this.bitmapHeight = bitmapHeight.toFloat()
    }
    ...
    @Synchronized
    fun setBitmapDimensions(bitmapWidth: Int, bitmapHeight: Int) {
        this.bitmapWidth = bitmapWidth.toFloat()
        this.bitmapHeight = bitmapHeight.toFloat()
    }
    ...
    fun translate(dx: Float, dy: Float) {
        ...
        val xmax = bitmapWidth / 2 + (surfaceWidth / 2f - SCROLL_BORDER) / surfaceScale
        ...
        val ymax = bitmapHeight / 2 + (surfaceHeight / 2f - SCROLL_BORDER) / surfaceScale
