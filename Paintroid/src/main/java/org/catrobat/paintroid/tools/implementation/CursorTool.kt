...
open class CursorTool(
    ...
    override fun drawShape(canvas: Canvas) {
        ...
        strokeWidth?.let {
            val cursorPartLength = it * 2
            val innerCircleRadius = brushStrokeWidth + it / 2f
            val outerCircleRadius = innerCircleRadius + it
            linePaint.apply {
            ...
            if (strokeCap == Cap.ROUND) {
                drawCircle(canvas, it, outerCircleRadius, innerCircleRadius)
            } else {
                drawRect(canvas, it, outerCircleRadius, innerCircleRadius)
            }

            // DRAW outer target lines
            linePaint.style = Paint.Style.FILL
            var startLineLengthAddition = it / 2f
            var endLineLengthAddition = cursorPartLength + it
            var lineNr = 0
            while (lineNr < CURSOR_LINES) {
                ...
                lineNr++
                startLineLengthAddition = it / 2f + cursorPartLength * lineNr
                endLineLengthAddition = it + cursorPartLength * (lineNr + 1f)
            }
        }
    }
