...
class DefaultWorkspace(
    ...
    override val currentLayerIndex: Int
        get() {
            layerModel.currentLayer?.let {
                return layerModel.getLayerIndexOf(it)
            }
            return -1
        }
    ...
    override fun getPixelOfCurrentLayer(coordinate: PointF): Int {
        if (coordinate.x >= 0 && coordinate.y >= 0 && coordinate.x < width && coordinate.y < height) {
            val bitmap = layerModel.currentLayer?.bitmap
            bitmap ?: return 0
            return bitmap.getPixel(coordinate.x.toInt(), coordinate.y.toInt())
        }
